...
If the main Agent omits `end_meeting`, a completed co-present turn is eligible
for the bounded fallback in [post-turn-coordinator.md](./post-turn-coordinator.md).
That fallback uses exact evidence and the unchanged interaction revision. It
does not handle arrival and cannot end a later meeting.

## 3. Durable State

SQLite migration 20 adds:
...
Only one canonical SMS session per character may be `co_present` at a time. RP
sandbox sessions are excluded from this uniqueness rule. Character or role
session deletion cascades to interaction state and events. Complete user-data
deletion removes both tables, and schema version 20 participates in normal
...
When `co_present`, the current scene projection may also enter the latest
volatile context even though the conversation mode remains SMS. Returning to
`remote` removes the scene projection on the next turn. Tool results communicate
a successful begin/end decision to the remainder of the current model loop, so
...
The model cannot submit a character or session ID. The bridge binds both to the
active Pi session. Disabling the module removes transition tools while retaining
stored state, injected state interpretation, and trusted UI controls. This lets
a user finish or repair an existing meeting without re-enabling model mutation.

## 6. World and Autonomy Coordination

When a canonical world place is selected, beginning a meeting updates the
...
The conversation header displays `角色私聊`, `约好见面`, or `见面中`, plus a
short location where relevant. Transition history is merged chronologically
into the visible transcript as compact non-message dividers. These events are
not written as user or character speech.
...
- A character cannot be canonically co-present in two private sessions.
