...
export function normalizeReminderPolicy(input: Partial<ReminderPolicy> | undefined, item: Pick<ScheduleItem, "kind" | "ownerType" | "startAt" | "allDay">): ReminderPolicy {
  ...
  if (input && Object.keys(input).some(key => !["enabled", "importance", "leadMinutes", "prepareMinutes", "channels"].includes(key))) throw new Error("unknown reminder policy field");
  ...
  const policy: ReminderPolicy = {
    ...
    channels: input?.channels ?? (importance === "important" ? ["in_app", "wechat"] : ["in_app"]),
  };
