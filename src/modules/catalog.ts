...
- \`send_subagent_message\`: queue a bounded follow-up turn on a completed job with a retained private transcript.
...
- Each initial or follow-up run persists a fenced owner claim, bounded transcript checkpoints, cumulative usage, and a write-ahead tool intent/result journal under its frozen budgets. Startup recovery restores committed results and resumes safe checkpoints; external or metered calls with an unknown outcome stay idle until the local control plane explicitly authorizes retry.
- Child tools are read-only: enabled Skill files, read-only Workspace and officeparser document conversion, configured Tavily Search, and configured Vision MCP.
